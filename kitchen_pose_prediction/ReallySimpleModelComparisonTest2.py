...
class SineWaveMaker(torch.utils.data.Dataset):
    ...
    def __getitem__(self, batch):
        ...
        return torch.tensor(
            [[0.5 + math.sin(index * 2 * math.pi / self.min_period / n) / 2 for n in range(1, self.dimension + 1)] for
             index in range(batch * self.batch_size, (batch + 1) * self.batch_size)])
...
def count_parameters(model):
    return sum(p.numel() for p in model.parameters() if
               p.requires_grad)  # TODO this may not count certain types of nested layers
...
class ModelWrapper:
    ...
    def get_losses_string(self):
        return "%s train loss: %1.6f %s test loss: %1.6f" % (
        self.name, self.train_losses[-1], self.name, self.test_losses[-1])
    ...
    def get_simple_losses_str(self):
        ...
        every epoch.
        """
        return ("%s train " + "%f " * len(self.train_losses) + "test " + "%f " * len(self.test_losses))[:-1] % (
        self.name, *self.train_losses, *self.test_losses)
    ...
    def save_models(self, filenames):
        for idx, network in enumerate(self.networks):
            if filenames[idx] is not None:
                torch.save(network.model.module.state_dict() if isinstance(network.model,
                                                                           torch.nn.DataParallel) else network.model.state_dict(),
                           filenames[idx])
...
if __name__ == "__main__":
    ...
    # network = ModelWrapper(
    #     TransformerEncoder(hidden_size, 8, input_size, num_layers, positional_embedding_max_len, dropout=0.0),
    #     "Transformer (0% dropout)", torch.optim.Adam, torch.nn.MSELoss(), {"lr": learning_rate})
    ...
    for model in networks.networks:
        ...
        schedulers.append(
            torch.optim.lr_scheduler.ReduceLROnPlateau(model._optimizer, mode='min', factor=0.9, patience=8, cooldown=8,
                                                       verbose=True, threshold=0.001, threshold_mode='rel'))
    print(networks.get_params_string())

    train_data = torch.utils.data.DataLoader(SineWaveMaker(timesteps=4096, min_period=128, dimension=1),
                                             batches_at_once, True, generator=torch.Generator(device=device))
    test_data = torch.utils.data.DataLoader(SineWaveMaker(timesteps=2048, min_period=128, dimension=1), batches_at_once,
                                            True, generator=torch.Generator(device=device))
