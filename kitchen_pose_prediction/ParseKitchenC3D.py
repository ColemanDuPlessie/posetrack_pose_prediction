...
if __name__ == "__main__":
    ...
    loaded_mocaps = load_and_prepare_mocaps(mocaps, batch_size, velo=True, framerate=30)
    print("Done loading!")
    loaded_mocaps = loaded_mocaps[torch.randperm(loaded_mocaps.shape[0])]
    print("Done shuffling!")
    for num, batch in enumerate(loaded_mocaps):
        torch.save(batch.clone(), "preprocessed_data/30fps_velo_1024/batch%s.pt" % str(num).zfill(4))
